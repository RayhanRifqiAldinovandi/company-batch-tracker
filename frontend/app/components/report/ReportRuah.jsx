      ...
      <div className="border">
                ...
                <ul tabIndex={0} className="dropdown-content z-[1] menu border mt-2 p-2 shadow bg-base-100 rounded-box w-32">
                  {paginatedDataRuah && (
                    <>
                      ...
                      <li>
                        <CSVLink data={paginatedDataRuah} filename={`Report_${startDate}_to_${endDate}.csv`} onClick={handleExportCSV}>
        ...
        <div className="mt-4 overflow-auto">
          <table id="report-table" className="report-table table table-zebra">
            ...
            <tbody>
              {startDate && endDate ? (
                paginatedDataRuah.length > 0 ? (
                  paginatedDataRuah.map((item) => (
                    <tr key={item.unique_number} className="uppercase">
                      ...
                      <td>{item.step_6}</td>
                      <td>
    {item.line_selection === "none" 
      ? (typeof item.line_record === "object" ? item.line_record : item.line_record || "N/A") 
      : (Object.keys(item.line_selection).length === 0 ? "N/A" : item.line_selection)
    }
  </td>
        ...
        <div className="flex justify-end items-center my-5 mr-2">
          ...
          <Stack spacing={2} marginLeft="10px">
            <Pagination count={Math.ceil(filteredDataRuah.length / pageSize)} page={currentPage} onChange={handlePageChange} renderItem={(item) => <PaginationItem slots={{ previous: ArrowBackIcon, next: ArrowForwardIcon }} {...item} />} />
          </Stack>
        </div>
      </div>
    </>
