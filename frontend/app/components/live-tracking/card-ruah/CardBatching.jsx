...
const CardBatching = () => {
  const { dataRuah, userType, totalDataBatchingRuah, isBatchTrackerOpen, setIsBatchTrackerOpen, selectedBatch, handleIconClick, handleReverseWoClick, handleSendButtonClick } = useContext(LiveTrackingContext);
      ...
      <div className="card bg-white border w-full min-w-[250px] rounded-xl shadow-md">
        <div className="px-4 h-[90px] flex flex-col justify-center">
          ...
          <div className="flex items-center justify-between mt-2">
            {/* Title */}
            <h1 className="uppercase font-bold">batching</h1>

            {/* Total */}
            <h2 className="bg-[#C5DFF8] px-2 text-[#3559E0] font-bold rounded-xl">{totalDataBatchingRuah}</h2>
          </div>
        </div>

        {/* Card */}
        <div className="min-h-[420px] max-h-[420px] overflow-auto">
          {dataRuah?.batching?.batching?.map((item) => (
