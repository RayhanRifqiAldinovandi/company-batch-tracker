      ...
      <div className="card bg-white border w-full min-w-[300px] rounded-xl shadow-md">
        <div className="px-4 h-[90px] flex flex-col justify-center">
          ...
          <div className="flex items-center justify-between mt-2">
            {/* Title */}
            <h1 className="uppercase font-bold">warehouse staging</h1>

            {/* Total */}
            <h2 className="bg-[#C5DFF8] px-2 text-[#3559E0] font-bold rounded-xl">{totalDataWarehouseStagingMinor}</h2>
          </div>
        </div>
          ...
          {handleMultipleBatchWarehouseStagingMinor && (
            <div className="border-b px-5 py-2">
              <div className="flex items-center">
                <input type="checkbox" className="mr-2" checked={isMasterCheckedWarehouseStagingMinor} onChange={handleMasterCheckboxChangeWarehouseStagingMinor} />
                <p className="uppercase">Select All</p>
              </div>
            </div>
          )}
          {dataMinor?.[warehouse_staging]?.map((item) => (
            <div key={item.uniqueNumber} className="border-b px-5 py-2 hover:bg-slate-200 transition duration-300">
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  {dataMinor?.[warehouse_staging]?.length > 1 && <input type="checkbox" className="mr-2" checked={isItemSelected(item)} onChange={() => handleCheckboxChange(item)} />}
        ...
        {handleMultipleBatchWarehouseStagingMinor && (userType === "super admin" || userType === "admin timbang" || userType === "operator timbang") && (
          <div className="p-4">
...
export default CardWarehouseStaging;
