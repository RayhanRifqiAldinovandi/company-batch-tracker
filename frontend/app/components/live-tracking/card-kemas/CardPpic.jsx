      ...
      <div className="card bg-white border w-full min-w-[300px] rounded-xl shadow-md">
        <div className="px-4 h-[90px] flex flex-col justify-center">
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            {/* Total */}
            <h2 className="bg-[#C5DFF8] px-2 text-[#3559E0] font-bold rounded-xl">{totalDataPpicKemas}</h2>
          </div>
        </div>
          ...
          {handleMultipleBatchPpicKemas && (
            <div className="border-b px-5 py-2">
              <div className="flex items-center">
                ...
                checked={isMasterCheckedPpicKemas} 
                onChange={handleMasterCheckboxChangePpicKemas} 
                />
                <p className="uppercase">Select All</p>
              </div>
            </div>
          )}
          {dataKemas?.ppic?.ppic?.map((item) => (
            <div key={item.uniqueNumber} className="border-b px-5 py-2 hover:bg-slate-200 transition duration-300">
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  {dataKemas.ppic?.ppic?.length > 1 && <input type="checkbox" className="mr-2" checked={isItemSelected(item)} onChange={() => handleCheckboxChange(item)} />}
        ...
        {handleMultipleBatchPpicKemas && (userType === "super admin" || department === "ppic") && (
          <div className="p-4">
            <button className="flex items-center justify-center bg-[#3559E0] hover:bg-[#8CABFF] text-white p-2 w-full rounded-xl transition duration-300" 
