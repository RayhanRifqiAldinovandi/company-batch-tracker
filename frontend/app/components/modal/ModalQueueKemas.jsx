...
const ModalQueueKemas = () => {
  const { handleReturnStaging, handleSendButtonClick, formatLineName, lineSortConfigKemas, sortedLineKemas, handleLineSortKemas, userType } = useContext(LiveTrackingContext);
              ...
              <table id="report-table" className="report-table table table-zebra table-pin-rows table-pin-cols w-full">
                <thead>
                  <tr>
                    ...
                    <th>Item Code</th>
                    <th>Batch Number</th>
                    <td className="flex items-center cursor-pointer capitalize" onClick={() => handleLineSortKemas("queue")}>
                      Line Selection
                      <div className="flex flex-col items-center ml-2">
                        <ArrowDropUpIcon className={`-m-2 ${lineSortConfigKemas.key === "batchNumber" && lineSortConfigKemas.direction === "ascending" ? "text-gray-500" : ""}`} />
                        <ArrowDropDownIcon className={`-m-2 ${lineSortConfigKemas.key === "batchNumber" && lineSortConfigKemas.direction === "descending" ? "text-gray-500" : ""}`} />
                      </div>
                    </td>
                    <th>Button</th>
                  </tr>
...
export default ModalQueueKemas;
